...
async function loadCoupons() {
  if (!container) return; // На всякий случай
  ...
  try {
    ...
    container.innerHTML = coupons.map(c => `
      <div class="product-card" style="border: 1px solid #ddd; padding: 20px; border-radius: 8px; text-align: center; background: #fff;">
        ...
        <button class="add-to-cart-btn" onclick="copyCode('${c.code}')" 
    ...
    container.innerHTML = `<p>Error loading coupons. ${err.message}. Please try again later.</p>`;
  }
}
